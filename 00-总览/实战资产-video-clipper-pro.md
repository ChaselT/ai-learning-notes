...
## 五·五、学完后的优化路线图（2026-07-31 学员确认）

**决策**：①本项目作为**支线**（学员判断：垂直个体户工具不契合多数互联网公司需求，靠里面的**技术点**说话而非产品本身）；②项目①照做，并**在项目①中顺带练 eval 体系**。

### 🔴 第 0 步（一切的前提）：eval 体系
30-50 条人工标注（方案采纳与否）+ 命中率基线。**没有度量衡，后面所有优化都是盲改**。

### 阶段 1 · 把"补偿"换成"正解"
- **结构化输出双轨制**：四级 JSON 修复层本质是在补偿没用 structured output → 支持 json_schema 的走严格模式，不支持的才降级修复层，降低"整轮作废"率
- **成本埋点**：加 usage 采集（现在完全没有 token 统计，成本控制是结构性的但无法量化）
- **Prompt 外置 + 版本号**：从 .py 硬编码字符串改为外置文件，为 A/B 与回滚铺路

### 阶段 2 · RAG ⭐ 护城河起点
- **历史爆款检索增强**：高转化切片（钩子/选段模式/配文）入向量库，分析时检索注入 → few-shot 从"通用范例"变成"自己验证过的成功案例"。**竞品能复刻代码，拿不到你的数据**
- **画面池语义匹配**：标签 Jaccard → embedding 语义匹配（文案语义 ↔ 画面语义）
- **极限词硬词表 → 语义判定**：降低误报（现词表含"最/超"单字），减少送进 LLM 的候选量 = 省钱提速
- **品类知识库**：不同品类的卖点/话术检索注入

### 阶段 3 · Agent + MCP 架构升级
- **MCP 补进度通知 + 升级 2026-07-28 无状态规范**（现 run_draft_workflow 同步阻塞几十分钟无反馈）→ 直通 [[开源选题池]] C1
- **11 步固定流水线 → Agent 编排**：时长不达标时可动态决策（重新分析 / 放宽约束 / 换素材 / 告知用户素材不足）
- **自建 tracing → 接 Langfuse**：成本/延迟/质量统一看板

### 阶段 4 · CV + 微调 ⭐ 最大技术跃迁 + 第二道护城河
- **画面打分：启发式 → 学习式**：用采纳数据训练画面质量打分模型，替代 Laplacian + 手调阈值
- **YOLO 训练服装检测**（领口/袖型/版型/图案）→ **项目⑤的完美载体，用真实业务数据而非玩具数据集**
- 💰 **QLoRA 微调降本**：用"采纳/未采纳"数据微调 7B 做初筛，砍掉大模型调用量。22G 显存正好够
- **本地 Whisper 替代火山 ASR**：成本归零 + 数据不出本地（对企业客户是卖点）

> **战略结论**：阶段 1、3 优化工程质量；**阶段 2、4 才是在造护城河**——它们把私有数据（哪些切片真卖货、哪些画面真被采用）变成别人复刻不了的能力。**护城河不是"代码复杂"，是"用数据训练出来的、只有你有的判断力"。**
