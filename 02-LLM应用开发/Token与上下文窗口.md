...
**"中文吃 token"要分两个维度看**（`o200k_base` 实测，2026-07 本机验证）：

| 维度 | 中文 | 英文 | 结论 |
|---|---|---|---|
| **字符效率** | 1.07 字符/token | 4.63 字符/token | 英文高 **4.3 倍**，老结论在此成立 |
| **同等语义**（同一段话的中英对照） | 635 tokens | 655 tokens | **基本持平**，老结论已不成立 |

原因：中文单字贵，但**信息密度高**——同样意思的中文字符数只有英文的 1/4，把劣势补了回来。GPT-3 时代（`p50k`/`cl100k`）中文一字常要 2~3 token，所以才有"中文贵 1.5~2 倍"的说法；`o200k_base` 与 Qwen 系 tokenizer 对中文优化后已经拉平。

粗略换算：**1000 token ≈ 750 英文单词 ≈ 1000 汉字**。

⚠️ 两个实践含义：①**上下文窗口的"能装多少字"和"能装多少信息"是两回事**——4096 token 约等于 4000 汉字或 19000 英文字符，但承载的信息量相当；②**别为省 token 把中文 prompt 翻成英文**，省不下来还引入翻译损耗。

另：token 价格取决于"高不高频"而非"是不是中文"——生僻字（龘靐爨）每字 2 token，冷门 emoji 每个 3 token，因为词表里没有它们、只能退回按 UTF-8 字节切；而高频 emoji（😀😂）只要 1 token。
...
> **本阶段第 2/11 课** | 上一篇：[[大语言模型工作原理速览]] | **下一篇：[[Chat Completions API]]**
